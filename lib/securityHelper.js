...
if (typeof CONFIG.PEM_PATH === 'string') {
  pem.config({
    pathOpenSSL: CONFIG.PEM_PATH,
  });
}
 ...
 */
module.exports.verifyJWS = (publicKeyPath, compactJWS, readKeyContents = utils.promisify(fs.readFile)) => {
  return new Promise(function (resolve, reject) {
    let keystore = jose.JWK.createKeyStore();

    readKeyContents(publicKeyPath)
      .then(publicKey => {
